...
#Save average valence data (averaged over 7 droplets) with time and also the no of droplet-droplet bonds over time 
output_file_averagevalence=os.path.splitext(trajectory_file)[0]+'.averagevalence.data'
output_file_dropletbondscount=os.path.splitext(trajectory_file)[0]+'.noofdropletbonds.data'

y=np.array([timestep_array,average_valences],dtype=object)
y.dump(output_file_averagevalence)

z=np.array([timestep_array,np.array(noofdropletbonds_allframes)],dtype=object)
z.dump(output_file_dropletbondscount)
