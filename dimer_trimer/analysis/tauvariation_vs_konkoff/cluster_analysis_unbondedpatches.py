...
trajectory_file=trajectory_file+"dimer_restl"+str(r0)+"_Nc"+str(Nclusters)+"_Np"+str(Np)+"_R"+str(R)+"_rB"+str(radiusB)+"_kAB"+str(kAB)+"_kon"+str(koninit)+"_koff"+str(koffinit)+"_dim"+str(dimension)+"_kspring"+str(kspring)+"_gammaA"+str(gammaA)+"_gammapatch"+str(gammapatch)+"_seed"+str(seed)+".allruns.gsd"
...
############################# loop over frames in frame_list to register bonded cluster pairs, num_dybonds, valence, and bondangle #############################
...
output_dict2={} # key: frame number, value: a dictionary of valence, bondangle, number of dybonds
...
print(num_unbonded_list[0][0],num_unbonded_list[0][-1])
