...
parser.add_argument('--selfavoidchain',action="store", nargs="*", default=1, dest="selfavoidchain", type=int)
...
parser.add_argument('--dir', action= "store", nargs="*", default='False', dest="dirname", type=str)
...
print("selfavoidchain:",results.selfavoidchain)
                                            ...
                                            for metropolis in results.metropolis:
                                                for kspring in results.kspring:
                                                    for zerodynbondlength in results.zerodynbondlength:
                                                        ...
                                                        koninit=float(koninit)
                                                        koffinit=float(koffinit)

                                                        if(results.selfavoidchain[0]==1):
                                                            polymertype='selfavoiding'
                                                        else:
                                                            polymertype='nonselfavoiding'
                                                        ...
                                                        for x in keys_clusters: 
                                                            if(x=='ABC'):
                                                                ...
                                                                new_data_clusters[x]['Nc']=0
                                                                new_data_clusters[x]['Np_per_sup']=0
                                                            if(idx==1):
                                                                break
                                                            idx+=1
                                                        ...
                                                        for y in keys_particles:
                                                            if(y=='dybond'):
                                                                for i in data_particles[y].keys():
                                                                    ...
                                                                    new_data_particles[y][key]['self_avoid_chain']=results.selfavoidchain[0]
                                                        ...
                                                        new_data_general['fileprefix']=str(simulationtype)+"_restl"+str(restlength)+"_Nc"+str(Nclusters)+"_Np"+str(Np_per_sup)+"_R"+str(R)+"_rB"+str(radiusB)+"_kAB"+str(kAB)+"_kon"+str(float(koninit))+"_koff"+str(float(koffinit))+"_dim"+str(dimension)+"_kspring"+str(kspring)+"_gammaA"+str(gammaA)+"_gammapatch"+str(gammapatch)+"_seed"+str(seed)

                                                        path=os.path.join(dirname,"seed"+str(seed),"restlength"+str(restlength))

                                                        #path=os.path.join(os.getcwd(),str(simulationtype),"gammaA"+str(gammaA)+"_gammapatch"+str(gammapatch),str(polymertype),"Nclusters"+str(Nclusters),"Np"+str(Np_per_sup),"R"+str(R),"radiusB"+str(radiusB),"kAB"+str(kAB),"epsilon"+str(epsilon),"dimension"+str(dimension),"kspring"+str(kspring),"seed"+str(seed),"restlength"+str(restlength))
