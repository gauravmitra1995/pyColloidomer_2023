...
def generate_plots_and_data(Nclusters,r0,kAB,radiusB,dimension,kspring,gammaA,gammapatch,clusterid):
    ...
    for filename in sorted(glob.glob(str(os.getcwd())+"/unbondedpatches_vs_time_data/"+str(simulationtype)+"_restl"+str(r0)+"_Nc"+str(Nclusters)+"_Np*_R*_rB"+str(radiusB)+"_kAB"+str(kAB)+"_eps"+str(epsilon)+"_dim"+str(dimension)+"_kspring"+str(kspring)+"_gammaA"+str(gammaA)+"_gammapatch"+str(gammapatch)+".averageallseeds.allruns.num_unbondedpatches.data"),key=lambda x:(int(os.path.basename(x).split("_")[3].replace('Np','')),float(os.path.basename(x).split("_")[4].replace('R','')))):
        data=np.load(filename,allow_pickle=True)
        R = float(os.path.basename(filename).split("_")[4].replace('R',''))
        Np = int(os.path.basename(filename).split("_")[3].replace('Np',''))
        label=(Np,R)
    ...
    for i in range(len(Nplist)):
        ...
        for j in range(len(Rlist)):
            ...
            p=ax.plot(timelist[::5],meanfractionlist[::5],marker='.',markersize=18,color=colors[r],label='$R = $'+str(label[1]))

            #this Np,R variation we are doing for high epsilon, so we do not need to do any other type of fitting other than double exponential fit

            #calculate the saturation fraction of unbonded binders
        ...
        print("Np:",Np," droplet:",droplet,saturationfractionlist)
        ...
        if(Nclusters==2 and clusterid==0):
            plt.title('Dimer: '+r'$N_b = $'+str(Np),fontsize=60)
        elif(Nclusters==2 and clusterid==1):
            plt.title('Dimer: '+r'$N_b = $'+str(Np),fontsize=60)
        elif(Nclusters==3 and clusterid==0):
            plt.title('Trimer (terminal droplet): '+r'$N_b = $'+str(Np),fontsize=60)
        elif(Nclusters==3 and clusterid==1):
            plt.title('Trimer (central droplet): '+r'$N_b = $'+str(Np),fontsize=60)
        ...
        fig.tight_layout()
        plt.savefig('final_figures/unbondedbinders_vs_time'+'_Nclusters'+str(Nclusters)+'_clusterid'+str(clusterid)+'_Np'+str(Np)+'_epsilon'+str(epsilon)+'_varyR.png',bbox_inches='tight')
        plt.close()

        #for each Np, save the saturation fraction list corresponding to each of the droplet radii
        output_saturationfraction=os.path.join(os.getcwd(),"saturation_data","saturationfraction_restl"+str(r0)+"_Nc"+str(Nclusters)+"_clusterid"+str(clusterid)+"_Np"+str(Np)+"_rB"+str(radiusB)+"_kAB"+str(kAB)+"_eps"+str(epsilon)+"_dim"+str(dimension)+"_kspring"+str(kspring)+"_gammaA"+str(gammaA)+"_gammapatch"+str(gammapatch)+".varyingRandNp.data")
