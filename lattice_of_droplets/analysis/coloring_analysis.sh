...
for task in $ntasks;do
   ...
   trajectory_file=$scriptdir/../simulation_setup/finalframes/${simulationtype}_restl${restlength}_Nc${Nclusters}_Np${Np}_R${R}_rB${radiusB}_areafrac${areafraction}_eps${epsilon}_kspring${kspring}_gammaA${gammaA}_gammapatch${gammapatch}_seed${seed}.finalframe.unwrap.gsd

   echo "trajectory_file=$trajectory_file"

   $wrapper python -u coloring_analysis_based_on_structure.py --trajectory_file $trajectory_file --dt $dt
   $wrapper python -u coloring_analysis_based_on_valence.py --trajectory_file $trajectory_file --dt $dt 
