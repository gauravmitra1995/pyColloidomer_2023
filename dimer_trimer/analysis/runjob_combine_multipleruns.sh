...
for task in $ntasks;do
   ...
   fileprefix=$scriptdir/../simulation_setup/${simulationtype}/gammaA${gammaA}_gammapatch${gammapatch}/selfavoiding/Nclusters${Nclusters}/Np${Np}/R${R}/radiusB${radiusB}/kAB${kAB}/epsilon${epsilon}/dimension${dimension}/kspring${kspring}/seed${seed}/restlength${restlength}/${simulationtype}_restl${restlength}_Nc${Nclusters}_Np${Np}_R${R}_rB${radiusB}_kAB${kAB}_eps${epsilon}_dim${dimension}_kspring${kspring}_gammaA${gammaA}_gammapatch${gammapatch}_seed${seed}

   echo $fileprefix

   $wrapper python -u join_gsdfiles_severalruns.py --fileprefix $fileprefix
